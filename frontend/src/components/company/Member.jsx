...
const Member = ({members, company, session}) => {
    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 lg:mb-6">
                ...
                <button className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-linear-to-r from-purple-500 to-pink-500 rounded-lg hover:shadow-lg hover:shadow-purple-500/50 transition-all">
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                            <div className="flex items-center gap-3 lg:gap-4 flex-1 min-w-0">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="text-base lg:text-lg font-semibold truncate">{member.user.name}</h3>
                                ...
                                (company.role === 'admin' && member.role !== 'owner' && member.user_id !== session.user_id)) && (
                                    <>
                                        <button 
                                            ...
                                            className="flex-1 sm:flex-none px-3 lg:px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white/70 hover:bg-white/10 transition-all text-xs lg:text-sm"
                                        >
                                            Change Role
                                        </button>
                                        <button 
                                            onClick={() => handleRemoveMember(member.user_id)}
                                            className="flex-1 sm:flex-none px-3 lg:px-4 py-2 bg-red-500/20 border border-red-500/30 rounded-lg text-red-300 hover:bg-red-500/30 transition-all text-xs lg:text-sm"
                                        >
                                            Remove
                                        </button>
                                    </>
